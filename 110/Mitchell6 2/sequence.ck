...
3::bpm.myDurations["tThirtysecond"] => d.delay;
1 => Rhodesgain.gain;
...
(1/.12)::bpm.myDurations["eighth"] => dur sync;
sync - (now % sync) => now;
...
{      
    ...
    1::bpm.myDurations["bar"] => now;
    spork ~ hatseq() @=> Shred @ ht;   
    2::bpm.myDurations["bar"] => now;
...
fun void tophats()
{
    ...
    while (true)
    {
        ...
        swing(1::bpm.myDurations["tSixteenth"], 0.15::bpm.myDurations["sixtyfourth"], (1/.12)::bpm.myDurations["tThirtysecond"], Std.rand2f(0.009, 0.2), "pull") => now;  
        hatcounter++;      
        0 => flag;
    }     
}
...
fun void rhodes()
{
    ...
    if (  pianocounter % 2 == 0 )
    {
        for (0 => int i; i < inversion.cap(); i++)
        {
            ...
            (4.12::bpm.myDurations["quarter"]) + Std.rand2f(.12, 1.12)::bpm.myDurations["tThirtysecond"] => now;       
        }
    }
}    
...
{    
    ...
    for (0 => int i ; i < melody.cap(); i++)
    {
        ...
        1 => clar.noteOn; 
        .13 => clar.gain;
        swing(Std.rand2(2, 5)::bpm.myDurations["eighth"], 1::bpm.myDurations["sixtyfourth"], 1::bpm.myDurations["tSixteenth"], Std.rand2f(0.12, 0.14), "push") => now;  
        1 => clar.stopBlowing;
    }
}