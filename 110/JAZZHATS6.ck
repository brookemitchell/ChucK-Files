...
8.33333333333333333333333334::bpm.myDurations["eighth"] => dur sync;
sync - (now % sync) => now;
...
{
    int pianocounter;
    Rhodenv.set( 400::ms, 620::ms, 0, 2000::ms );  
    if ( pianocounter % 2 == 0 )
    {
        for (0 => int i; i < inversion.cap(); i++)
        {
            jazz.generateChord(roots[i], chordquality[i], inversion[i]);
            Std.mtof(jazz.chord[0])-24 => voc0.freq;
            1 => Rhodenv.keyOn;
            1 => voc0.noteOn;
            Std.mtof(jazz.chord[1])-24 => voc1.freq;
            .7 => voc1.noteOn;
            Std.mtof(jazz.chord[2])-24 => voc2.freq;
            .9 => voc2.noteOn;
            Std.mtof(jazz.chord[3])-24 => voc3.freq;
            .8 => voc3.noteOn;
